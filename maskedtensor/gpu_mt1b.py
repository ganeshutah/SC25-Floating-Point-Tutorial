...
x = torch.tensor([-10., -5, 0, 5, 10, 50, 60, 70, 80, 90, 100], requires_grad=True).to('cuda')
...
x = torch.tensor([-10., -5, 0, 5, 10, 50, 60, 70, 80, 90, 100], requires_grad=True).to('cuda')
mask = x < 0
mx = masked_tensor(x, mask, requires_grad=True).to('cuda')
my = masked_tensor(torch.ones_like(x), ~mask, requires_grad=True).to('cuda')
